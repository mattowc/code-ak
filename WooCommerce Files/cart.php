...
<form id='Cart66CartForm' action="" method="post">
  ...
  <thead>
    <tr>
      ...
      <th class="align-center"><?php _e( 'Quantity' , 'cart66' ); ?></th>
      <th class="align-right"><?php _e( 'Item Price' , 'cart66' ); ?></th>
      <th class="align-right"><?php _e( 'Item Total' , 'cart66' ); ?></th>
    </tr>
  </thead>
  <tbody>
    <?php foreach($items as $itemIndex => $item): ?>
      ...
      <tr>
        ...
        <?php else: ?>
          <td class="align-center <?php if($item->hasAttachedForms()) { echo "noBottomBorder"; } ?>"><?php echo $item->getQuantity() ?></td>
        <?php endif; ?>
        <td class="align-right <?php if($item->hasAttachedForms()) { echo "noBottomBorder"; } ?>"><?php echo $item->getProductPriceDescription(); ?></td>
        <td class="align-right <?php if($item->hasAttachedForms()) { echo "noBottomBorder"; } ?>"><?php echo CART66_CURRENCY_SYMBOL ?><?php echo number_format($price, 2);?></td>
      </tr>
    ...
    <tr class="subtotal">
      ...
      <td class='strong align-right'><?php echo CART66_CURRENCY_SYMBOL ?><?php echo number_format($subtotal, 2); ?></td>
    </tr>
    ...
    <tr class="shipping">
      ...
      <td class="strong align-right"><?php echo CART66_CURRENCY_SYMBOL ?><?php echo $shipping ?></td>
    </tr>
    <?php endif; ?>
    
    <?php if($promotion): ?>
      <tr class="coupon">
        <td colspan="3" class="alignRight strong"><?php _e( 'Referral code' , 'cart66' ); ?> 
        <?php 
          if($promotion->name){ 
            echo "(" .$promotion->name .")"; 
          }
        ...
        ?>:</td>
        <td class="strong align-right">-&nbsp;<?php echo  CART66_CURRENCY_SYMBOL;
    ...
    <tr class="tax-row <?php echo $tax > 0 ? 'show-tax-row' : 'hide-tax-row'; ?>">
      ...
      <td class="strong tax-amount align-right"><?php echo CART66_CURRENCY_SYMBOL ?><?php echo number_format($tax, 2); ?></td>
    </tr>
      ...
      <tr class="total">
        ...
        <td class="alignRight" colspan='2'>
          <?php if($fullMode && Cart66Common::activePromotions()): ?>
            <p class="haveCoupon"><?php _e( 'Enter referral code' , 'cart66' ); ?></p>
          <?php if(Cart66Session::get('Cart66PromotionErrors')):
            ...
            <div id="updateCart">
              <?php if($cartImgPath && Cart66Common::urlIsLIve($applyCouponImg)): ?>
                <input class="Cart66ApplyCouponButton" type="image" src='<?php echo $applyCouponImg ?>' value="<?php _e( 'Add Referral' , 'cart66' ); ?>" name="updateCart"/>
              <?php else: ?>
                <input type='submit' name='updateCart' value='<?php _e( 'Apply Referral' , 'cart66' ); ?>' class="Cart66ApplyCouponButton Cart66ButtonSecondary" />
              <?php endif; ?>
            </div>
          <?php endif; ?>
        </td>
        ...
        <td class="strong grand-total-amount align-right">
          <?php 
